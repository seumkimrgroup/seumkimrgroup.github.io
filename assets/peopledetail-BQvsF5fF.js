import{f,e as a}from"./util.data-QhEPzZg9.js";import{f as E,b as k}from"./component.iconlinks-DnUx7pjz.js";import{c as w}from"./component.card.publication-zQcQtc1R.js";let g=[];function e(t){return t.status==="former"}function I(t){const n=String(t.name||"").toLowerCase();return g.filter(i=>String(i.authors||"").toLowerCase().includes(n)).sort((i,s)=>String(s.date||"").localeCompare(String(i.date||"")))}function u(t,n){if(!Array.isArray(t.description))return[];const i=t.description.find(s=>String((s==null?void 0:s.title)||"").trim().toUpperCase()===n.toUpperCase());return!i||!Array.isArray(i.content)?[]:i.content.filter(s=>String(s||"").trim()!=="")}function p(t,n){if(!n.length)return"";const i=`<p>${n.map(a).join(` <br>
            `)}</p>`;return`
    <div class="stack--sm">
      <h4>${a(t)}</h4>
      ${i}
