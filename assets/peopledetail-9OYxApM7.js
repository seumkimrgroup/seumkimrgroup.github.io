import{a as e,i as t,n,r,t as i}from"./util.data-BJfEJwti.js";import{t as a}from"./component.card.publication-DMaMfMJ2.js";import{i as o,t as s}from"./component.iconlinks-1LXe0QC5.js";e(),t(),r();var c=[];function l(e){return e.status===`former`}function u(e){let t=String(e.name||``).toLowerCase();return c.filter(e=>String(e.authors||``).toLowerCase().includes(t)).sort((e,t)=>String(t.date||``).localeCompare(String(e.date||``)))}function d(e,t){if(!Array.isArray(e.description))return[];let n=e.description.find(e=>String(e?.title||``).trim().toUpperCase()===t.toUpperCase());return!n||!Array.isArray(n.content)?[]:n.content.filter(e=>String(e||``).trim()!==``)}function f(e,t){if(!t.length)return``;let n=`<p>${t.map(i).join(` <br>
            `)}</p>`;return`
    <div class="stack--8">
      <h4>${i(e)}</h4>
      ${n}
