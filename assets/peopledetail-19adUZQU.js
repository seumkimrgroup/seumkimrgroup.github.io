import{i as C,d as E,g as k,f,e as a}from"./util.data-diC6Jxad.js";import{f as I,r as L}from"./component.iconlinks-K1BlDMwZ.js";import{c as P}from"./component.card.publication-PvHlwzSj.js";C();E();k();let h=[];function o(t){return t.status==="former"}function T(t){const n=String(t.name||"").toLowerCase();return h.filter(i=>String(i.authors||"").toLowerCase().includes(n)).sort((i,s)=>String(s.date||"").localeCompare(String(i.date||"")))}function u(t,n){if(!Array.isArray(t.description))return[];const i=t.description.find(s=>String((s==null?void 0:s.title)||"").trim().toUpperCase()===n.toUpperCase());return!i||!Array.isArray(i.content)?[]:i.content.filter(s=>String(s||"").trim()!=="")}function p(t,n){if(!n.length)return"";const i=`<p>${n.map(a).join(` <br>
            `)}</p>`;return`
    <div class="stack--8">
      <h4>${a(t)}</h4>
      ${i}
    </div>
  `}function H(t){return Array.isArray(t.links)&&t.links.length>0?t.links.filter(n=>n&&n.type&&n.value):t.email?[{type:"email",value:t.email}]:[]}function N(t){return L(H(t))}function M(t,n){const i=T(t),s=p("Education",u(t,"EDUCATION")),e=p("Research Interest",u(t,"RESEARCH INTERESTS")),c=o(t)?"":I(t.role),g=o(t)?"":String(t.currentAffiliation||"").trim(),l=[c,g].filter(Boolean),S=l.length?`<p class="color-secondary">${l.map(a).join(` <br>
          `)}</p>`:"",d=String(t.nextAffiliation||"").trim(),v=o(t)&&d?`<div class="row"><p class="color-secondary" style="white-space:nowrap">JOINED AT</p><p>${a(d)}</p></div>`:"",$=o(t)?"":t.image?`<div class="media"><img src="${t.image}" alt="${a(t.name)}"></div>`:'<div class="media"></div>',y=o(t)?"stack--16":"split",A=o(t)?`${s}`:`${s}${e}`;if(n.innerHTML=`
    <div class="${y}">
      ${$}
