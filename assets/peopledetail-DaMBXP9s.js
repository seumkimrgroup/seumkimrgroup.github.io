import{f,e}from"./util.data-BiTpGavE.js";import{f as E,b as k}from"./component.iconlinks-CK4ifhxe.js";import{c as w}from"./component.card.publication-_8t26e66.js";import"./icons--JfbYone.js";let g=[];function a(t){return t.status==="former"}function I(t){const n=String(t.name||"").toLowerCase();return g.filter(i=>String(i.authors||"").toLowerCase().includes(n)).sort((i,s)=>String(s.date||"").localeCompare(String(i.date||"")))}function u(t,n){if(!Array.isArray(t.description))return[];const i=t.description.find(s=>String((s==null?void 0:s.title)||"").trim().toUpperCase()===n.toUpperCase());return!i||!Array.isArray(i.content)?[]:i.content.filter(s=>String(s||"").trim()!=="")}function p(t,n){if(!n.length)return"";const i=`<p>${n.map(e).join(` <br>
            `)}</p>`;return`
    <div class="stack--sm">
      <h4>${e(t)}</h4>
      ${i}
    </div>
  `}function L(t){return Array.isArray(t.links)&&t.links.length>0?t.links.filter(n=>n&&n.type&&n.value):t.email?[{type:"email",value:t.email}]:[]}function P(t){return k(L(t))}function m(t,n){const i=I(t),s=p("Education",u(t,"EDUCATION")),o=p("Research Interest",u(t,"RESEARCH INTERESTS")),c=a(t)?"":E(t.role),h=a(t)?"":String(t.currentAffiliation||"").trim(),l=[c,h].filter(Boolean),S=l.length?`<p class="text-secondary">${l.map(e).join(` <br>
