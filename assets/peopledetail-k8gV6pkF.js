import{f,e as o}from"./util.data-DydlDVKe.js";import{f as C,b as E}from"./component.iconlinks-Bm_cyZnn.js";import{c as k}from"./component.card.publication-C1gFosbb.js";let h=[];function e(t){return t.status==="former"}function I(t){const n=String(t.name||"").toLowerCase();return h.filter(i=>String(i.authors||"").toLowerCase().includes(n)).sort((i,s)=>String(s.date||"").localeCompare(String(i.date||"")))}function u(t,n){if(!Array.isArray(t.description))return[];const i=t.description.find(s=>String((s==null?void 0:s.title)||"").trim().toUpperCase()===n.toUpperCase());return!i||!Array.isArray(i.content)?[]:i.content.filter(s=>String(s||"").trim()!=="")}function p(t,n){if(!n.length)return"";const i=`<p>${n.map(o).join(` <br>
            `)}</p>`;return`
    <div class="stack--sm">
      <h4>${o(t)}</h4>
      ${i}
