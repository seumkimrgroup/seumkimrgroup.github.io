...
function renderProfessorInline(member) { piContainer.innerHTML = `${renderProfileHeader(member, true)}<div class="member-detail-sections member-detail-sections--pi">${renderAllSections(member)}</div>`; const pubList = document.getElementById("pi-publication-list"); if (pubList) getMemberPublications(member).forEach((pub) => pubList.appendChild(createPublicationCard(pub))); }
...
function renderMemberDetail(member) {
  ...
  if (isFormer(member)) {
    ...
    detailSections = `<div class="member-detail-sections">${educationSection}${researchSection}${isProfessor(member) ? "" : renderDetailPublications(member)}</div>`;
  }
