import{e as c,f as k}from"./util.data-DA4hmx_G.js";import{c as V}from"./component.card.publication-ooYpUucj.js";const C={JQL:"project",TACTILE:"project",SW:"project","LC-EO":"topic","LC-COMP":"topic","ML-FEA":"topic",PSLC:"method",CLCE:"method",LCE:"method",PIC:"method",THERM:"method",MECH:"method",ELEC:"method",FEATURED:"featured"},q=["project","topic","method","featured"];function x(e){return!e||!e.length?"":`<div class="group">${[...e].sort((n,a)=>{const r=q.indexOf(C[n]??""),s=q.indexOf(C[a]??"");return(r===-1?99:r)-(s===-1?99:s)}).map(n=>{const a=C[n];return`<span class="${a?`tag tag--${a}`:"tag"}">${c(n)}</span>`}).join("")}</div>`}const T=document.querySelector("#projects .carousel__track"),E=document.querySelector("#projects .carousel-nav");async function W(){if(!T)return;const e=await k("/assets/data/projects.json");if(!Array.isArray(e)||e.length===0)return;T.innerHTML="",e.map(r=>{const s=document.createElement("div");return s.className="carousel__panel",s.style.backgroundImage=`url(${r.background||""})`,s.innerHTML=`
          <div class="inner">
            <div class="stack">
