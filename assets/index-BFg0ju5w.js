import{e as c,f as L}from"./data-BpF6WWch.js";import{c as V}from"./card.publication-Bz36yTV7.js";const A={JQL:"project",TACTILE:"project",SW:"project","LC-EO":"topic","LC-COMP":"topic","ML-FEA":"topic",PSLC:"method",CLCE:"method",LCE:"method",PIC:"method",THERM:"method",MECH:"method",ELEC:"method",FEATURED:"featured"},N=["project","topic","method","featured"];function M(e){return!e||!e.length?"":`<div class="group">${[...e].sort((n,a)=>{const r=N.indexOf(A[n]??""),s=N.indexOf(A[a]??"");return(r===-1?99:r)-(s===-1?99:s)}).map(n=>{const a=A[n];return`<span class="${a?`tag tag--${a}`:"tag"}">${c(n)}</span>`}).join("")}</div>`}const _=document.querySelector("#projects .carousel__track"),E=document.querySelector("#projects .carousel-nav");async function W(){if(!_)return;const e=await L("/assets/data/projects.json");if(!Array.isArray(e)||e.length===0)return;_.innerHTML="",e.map(r=>{const s=document.createElement("div");return s.className="carousel__panel",s.style.backgroundImage=`url(${r.background||""})`,s.innerHTML=`
          <div class="inner">
            <div class="stack">
  ...
  `,e.addEventListener("click",t=>{t.target===e&&w()}),e.querySelector(".modal-close").addEventListener("click",w),document.addEventListener("keydown",t=>{t.key==="Escape"&&w()}),document.body.appendChild(e),e}function w(){f&&(f.classList.remove("is-open"),document.body.classList.remove("modal-open"))}function K(e){f||(f=Q());const t=f.querySelector(".media img");t.src=e.image||"",t.alt=c(e.title||"");const n=f.querySelector(".info h6");n.textContent=e.type?e.type.charAt(0).toUpperCase()+e.type.slice(1):"",n.hidden=!e.type;const a=f.querySelector(".info h3");a.textContent=e.title||"",a.hidden=!e.title,f.querySelector(".info > p").textContent=e.description||"",f.querySelector(".links-wrap").innerHTML=J(e.links),f.classList.add("is-open"),document.body.classList.add("modal-open")}const Z='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true" class="card--recruit__chevron"><path stroke-linecap="round" stroke-linejoin="round" d="M6 9l6 6 6-6"/></svg>';function q(e,t){const n=e.classList.contains("is-open");document.querySelectorAll(".card--recruit.is-open").forEach(a=>{a.classList.remove("is-open"),a.querySelector(".card--recruit__btn").setAttribute("aria-expanded","false")}),n||(e.classList.add("is-open"),t.setAttribute("aria-expanded","true"))}function ee(e){const t=document.createElement("div");t.className="card card--recruit",t.innerHTML=`
  ...
  `;const r=n.querySelector(":scope > .stack"),s=ae(a,t);return s.length?s.forEach(d=>r.appendChild(V(d))):r.innerHTML="<p>No related publications found.</p>",n}function oe(e,t){const n=document.createElement("button");n.type="button",n.className="btn areas-tab",n.dataset.index=String(t);const r=(Array.isArray(e.tag)?e.tag:[]).find(s=>A[s]==="topic")??e.title;return n.innerHTML=`<span class="tab-short">${c(r)}</span><span class="tab-full">${c(e.title)}</span>`,n}let T=null;function j(e){const t=C.querySelectorAll(".areas-tab"),n=m.querySelectorAll(".carousel__panel");m.style.transform=`translateX(-${e*100}%)`,t.forEach((r,s)=>r.classList.toggle("active",s===e));const a=n[e];a&&(T&&T.disconnect(),T=new ResizeObserver(()=>{m.parentElement.style.height=a.offsetHeight+"px"}),T.observe(a))}async function ie(){if(!(!C||!m))try{const[e,t]=await Promise.all([L("/assets/data/contents.json"),L("/assets/data/publications.json")]),n=e.filter(a=>a.type==="topic");if(!n.length){m.innerHTML='<article class="carousel__panel"><p>No research topics found.</p></article>';return}C.innerHTML="",m.innerHTML="",n.forEach((a,r)=>{const s=oe(a,r),d=se(a,t);s.addEventListener("click",()=>j(r)),C.appendChild(s),m.appendChild(d)}),j(0)}catch(e){console.error(e),m.innerHTML='<article class="carousel__panel"><p>Research areas could not be loaded.</p></article>'}}ie();
