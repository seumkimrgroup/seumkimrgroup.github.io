import{e as c,f as k}from"./util.data-DV1ssOpE.js";import{c as V}from"./component.card.publication-CwTCDVBw.js";const C={JQL:"project",TACTILE:"project",SW:"project","LC-EO":"topic","LC-COMP":"topic","ML-FEA":"topic",PSLC:"method",CLCE:"method",LCE:"method",PIC:"method",THERM:"method",MECH:"method",ELEC:"method",FEATURED:"featured"},N=["project","topic","method","featured"];function M(e){return!e||!e.length?"":`<div class="group">${[...e].sort((n,a)=>{const r=N.indexOf(C[n]??""),s=N.indexOf(C[a]??"");return(r===-1?99:r)-(s===-1?99:s)}).map(n=>{const a=C[n];return`<span class="${a?`tag tag--${a}`:"tag"}">${c(n)}</span>`}).join("")}</div>`}const T=document.querySelector("#projects .carousel__track"),E=document.querySelector("#projects .carousel-nav");async function W(){if(!T)return;const e=await k("/assets/data/projects.json");if(!Array.isArray(e)||e.length===0)return;T.innerHTML="",e.map(r=>{const s=document.createElement("div");return s.className="carousel__panel",s.style.backgroundImage=`url(${r.background||""})`,s.innerHTML=`
    ...
    <div class="stack--xs">
      ${e.type?`<h6 class="${Y[e.type]||""}">${c(e.type.charAt(0).toUpperCase()+e.type.slice(1))}</h6>`:""}
      ${e.title?`<h4>${c(e.title)}</h4>`:""}
      <p>${c(e.description||"")}</p>
      ${z(e.links)}
    </div>
  `,t}let f=null;function Q(e){if(!Array.isArray(e)||!e.length)return"";const t=e.map(a=>`<a href="${c(a.url)}" target="_blank" rel="noopener noreferrer">${c(a.text)}</a>`);return`<p class="text-secondary">More information:  ${t.length===1?t[0]:t.length===2?`${t[0]} and ${t[1]}`:`${t.slice(0,-1).join(", ")}, and ${t[t.length-1]}`}.</p>`}function K(){const e=document.createElement("div");return e.className="modal-overlay",e.innerHTML=`
  ...
  `,e.addEventListener("click",t=>{t.target===e&&w()}),e.querySelector(".modal-close").addEventListener("click",w),document.addEventListener("keydown",t=>{t.key==="Escape"&&w()}),document.body.appendChild(e),e}function w(){f&&(f.classList.remove("is-open"),document.body.classList.remove("modal-open"))}function Z(e){f||(f=K());const t=f.querySelector(".media img");t.src=e.image||"",t.alt=c(e.title||"");const n=f.querySelector(".stack--sm h6");n.textContent=e.type?e.type.charAt(0).toUpperCase()+e.type.slice(1):"",n.hidden=!e.type;const a=f.querySelector(".stack--sm h3");a.textContent=e.title||"",a.hidden=!e.title,f.querySelector(".stack--sm > p").textContent=e.description||"",f.querySelector(".links-wrap").innerHTML=Q(e.links),f.classList.add("is-open"),document.body.classList.add("modal-open")}const ee='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M6 9l6 6 6-6"/></svg>';function q(e,t){const n=e.classList.contains("is-open");document.querySelectorAll(".card--recruit.is-open").forEach(a=>{a.classList.remove("is-open"),a.querySelector(".split > button").setAttribute("aria-expanded","false")}),n||(e.classList.add("is-open"),t.setAttribute("aria-expanded","true"))}function te(e){const t=document.createElement("div");t.className="card card--recruit",t.innerHTML=`
    <div class="split">
      <h5>${c(e.program||"")}</h5>
  ...
  `;const r=n.querySelector(":scope > .stack"),s=se(a,t);return s.length?s.forEach(d=>r.appendChild(V(d))):r.innerHTML="<p>No related publications found.</p>",n}function ie(e,t){const n=document.createElement("button");n.type="button",n.className="btn areas-tab",n.dataset.index=String(t);const r=(Array.isArray(e.tag)?e.tag:[]).find(s=>C[s]==="topic")??e.title;return n.innerHTML=`<span class="tab-short">${c(r)}</span><span class="tab-full">${c(e.title)}</span>`,n}let A=null;function j(e){const t=_.querySelectorAll(".areas-tab"),n=m.querySelectorAll(".carousel__panel");m.style.transform=`translateX(-${e*100}%)`,t.forEach((r,s)=>r.classList.toggle("active",s===e));const a=n[e];a&&(A&&A.disconnect(),A=new ResizeObserver(()=>{m.parentElement.style.height=a.offsetHeight+"px"}),A.observe(a))}async function ce(){if(!(!_||!m))try{const[e,t]=await Promise.all([k("/assets/data/contents.json"),k("/assets/data/publications.json")]),n=e.filter(a=>a.type==="topic");if(!n.length){m.innerHTML='<article class="carousel__panel"><p>No research topics found.</p></article>';return}_.innerHTML="",m.innerHTML="",n.forEach((a,r)=>{const s=ie(a,r),d=oe(a,t);s.addEventListener("click",()=>j(r)),_.appendChild(s),m.appendChild(d)}),j(0)}catch(e){console.error(e),m.innerHTML='<article class="carousel__panel"><p>Research areas could not be loaded.</p></article>'}}ce();
