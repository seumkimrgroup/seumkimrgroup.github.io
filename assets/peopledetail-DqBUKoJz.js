import{f,e as a}from"./util.data-0wJgrHOB.js";import{f as w,b as E}from"./component.iconlinks-JfDKBhDl.js";import{c as k}from"./component.card.publication-m1L7yMJt.js";let h=[];function e(t){return t.status==="former"}function I(t){const n=String(t.name||"").toLowerCase();return h.filter(i=>String(i.authors||"").toLowerCase().includes(n)).sort((i,s)=>String(s.date||"").localeCompare(String(i.date||"")))}function p(t,n){if(!Array.isArray(t.description))return[];const i=t.description.find(s=>String((s==null?void 0:s.title)||"").trim().toUpperCase()===n.toUpperCase());return!i||!Array.isArray(i.content)?[]:i.content.filter(s=>String(s||"").trim()!=="")}function u(t,n){if(!n.length)return"";const i=`<p>${n.map(a).join(` <br>
            `)}</p>`;return`
    <div class="stack--sm">
      <h4>${a(t)}</h4>
      ${i}
    </div>
  `}function L(t){return Array.isArray(t.links)&&t.links.length>0?t.links.filter(n=>n&&n.type&&n.value):t.email?[{type:"email",value:t.email}]:[]}function P(t){return E(L(t))}function H(t,n){const i=I(t),s=u("Education",p(t,"EDUCATION")),o=u("Research Interest",p(t,"RESEARCH INTERESTS")),c=e(t)?"":w(t.role),g=e(t)?"":String(t.currentAffiliation||"").trim(),l=[c,g].filter(Boolean),S=l.length?`<p class="text-secondary">${l.map(a).join(` <br>
    ...
    ${A}
  `,i.length){const r=document.createElement("div");r.className="stack",r.innerHTML="<h4>Publications</h4>",i.forEach(C=>r.appendChild(k(C,{showMeta:!1}))),n.appendChild(r)}}async function M(){const t=new URLSearchParams(window.location.search).get("id");if(!t)return;const n=document.getElementById("profile");if(!n)return;const[i,s]=await Promise.all([f("/assets/data/people.json"),f("/assets/data/publications.json")]);h=s;const o=i.find(c=>c.slug===t);o&&H(o,n)}M();
