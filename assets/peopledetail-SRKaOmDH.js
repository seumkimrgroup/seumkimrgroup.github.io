import{f as d,e}from"./util.data-gk1xnSV2.js";import{f as w,b as C}from"./component.iconlinks-B1RVS-cf.js";import{c as y}from"./component.card.publication-T2COrNId.js";function a(t){return t.status==="former"}function k(t){const n=String(t.name||"").toLowerCase();return publications.filter(i=>String(i.authors||"").toLowerCase().includes(n)).sort((i,s)=>String(s.date||"").localeCompare(String(i.date||"")))}function u(t,n){if(!Array.isArray(t.description))return[];const i=t.description.find(s=>String((s==null?void 0:s.title)||"").trim().toUpperCase()===n.toUpperCase());return!i||!Array.isArray(i.content)?[]:i.content.filter(s=>String(s||"").trim()!=="")}function f(t,n){if(!n.length)return"";const i=`<p>${n.map(e).join(` <br>
            `)}</p>`;return`
    <div class="stack--sm">
      <h4>${e(t)}</h4>
      ${i}
